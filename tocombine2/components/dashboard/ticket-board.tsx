      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {ambiguousTickets.map((ticket) => (
            <AmbiguousTicketCard key={ticket.id} ticket={ticket} />
          ))}
        </div>
      </div>
    </div>
...
function TicketCard({ ticket }: { ticket: KnownTicket }) {
  return (
    <article className="rounded-2xl border border-white/10 bg-white/5 p-4 shadow-inner shadow-black/40">
      ...
      <p className="mt-2 text-lg font-semibold text-white">{ticket.title}</p>
...
// Ambiguous Ticket Card Component
function AmbiguousTicketCard({ ticket }: { ticket: AmbiguousTicket }) {
  return (
    <article className="rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 p-4">
