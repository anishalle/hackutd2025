...
type DisposableRoomEnvironment = RoomEnvironment & { dispose?: () => void };
...
const MARKERS: MarkerDef[] = [
  {
    id: 'cooling_manifold',
    label: 'Liquid cooling manifold',
    color: '#00f5d4',
    position: [0, 0.85, 0.28],
  },
  {
    id: 'nvlink_fabric',
    label: 'NVLink & NVSwitch fabric',
    color: '#64b5ff',
    position: [0, 0.55, 0],
  },
  {
    id: 'h100_sled',
    label: 'H100 compute sled stack',
    color: '#ff6b00',
    position: [0, 0.15, 0.07],
  },
  {
    id: 'telemetry_drawer',
    label: 'Telemetry + BMC drawer',
    color: '#c084fc',
  ...
  {
    id: 'uplink_panel',
    label: 'QSFP-DD uplink panel',
    color: '#ff4d6d',
    position: [0.22, 0.45, -0.2],
  },
  {
    id: 'fiber_trunk',
    label: 'Spine-facing fiber trunk',
    color: '#2ec4b6',
    position: [-0.2, 0.1, -0.3],
  },
  {
    id: 'aux_pump',
    label: 'Aux loop pump plate',
    color: '#ffd6ff',
...
const ThreeScene: React.FC<ThreeSceneProps> = ({
  ...
  useEffect(() => {
    ...
    labelRenderer.setSize(widthPx, heightPx);
    ...
    const env: DisposableRoomEnvironment = new RoomEnvironment();
    const envMap = pmrem.fromScene(env, 0.04).texture;
    scene.environment = envMap;

    // --- Camera & controls ---
    const camera = new THREE.PerspectiveCamera(60, widthPx / heightPx, 0.1, 2000);
  ...
  const widthStyle = normalizeSize(width, '74vw');
  const heightStyle = normalizeSize(height, '74vh');

  return <div ref={containerRef} style={{ position: 'relative', width: widthStyle, height: heightStyle, maxWidth: '100%' }} />;
};
