...
export function FloatingIslandNav({ links }: FloatingIslandNavProps) {
  ...
  return (
    <div className="fixed left-0 right-0 top-4 z-50 flex w-full items-center justify-center px-4">
      <div className="flex w-full max-w-7xl items-center justify-between gap-4">
        <nav className="flex flex-1 items-center justify-between gap-1 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white shadow-[0_20px_60px_-35px_rgba(56,189,248,0.9)] backdrop-blur-2xl">
          {links.map((link) => {
            const isActive =
              pathname === link.href ||
